...
fn trajectory_from_recorded_session_validates_and_writes() {
    ...
    assert!(trajectory.steps.iter().any(|step| {
        step.source == "agent" && step.message.contains("README looks good")
    }));
    assert_eq!(
        trajectory
            .final_metrics
    ...
    assert!(inspect_message_stream_jsonl(&ok!(fs::read_to_string(
...
fn orphan_tool_call_without_agent_message_creates_run_step() {
    ...
    assert!(step.tool_calls.as_ref().is_some_and(|calls| {
        calls
            .iter()
            .any(|call| call.tool_call_id == "call-a")
    }));
}
...
{"type":"usage","run":"run-b","input_tokens":23,"output_tokens":5}"#,
    ...
    assert_eq!(
        run_a.metrics.as_ref().and_then(|metrics| metrics.prompt_tokens),
        Some(11)
    );
