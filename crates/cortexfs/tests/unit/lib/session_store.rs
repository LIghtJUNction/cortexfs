...
fn session_store_flush_writes_readable_parquet_schema() {
    ...
    let payload = batch
        ...
        .downcast_ref::<StringArray>()
        .map(|array| array.value(0));
    assert_eq!((stream, ordinal, payload), (Some("messages"), Some(0), Some(line)));
}
...
fn session_store_export_writes_readable_parquet_dataset() {
    ...
    let mut rows = 0_i64;
    for path in &files {
        let builder = ok!(ParquetRecordBatchReaderBuilder::try_new(ok!(fs::File::open(path))));
...
fn session_store_accepts_256kib_payload_and_rejects_next_byte() {
    ...
    let first = super::columnar::append(
        &session,
        super::columnar::Stream::Messages,
        &[&accepted],
    );
    let second = super::columnar::append(
        &session,
        super::columnar::Stream::Messages,
        &[&rejected],
    );
...
fn session_store_tightens_store_permissions() {
    ...
    for directory in [&store, &store.join("data"), &store.join("index")] {
        assert_eq!(ok!(fs::metadata(directory)).permissions().mode() & 0o777, 0o700);
    }
    for file in [
        store.join("lock"),
...
fn session_store_export_copy_failure_is_atomic_and_retryable() {
    ...
    let leaked_temp = ok!(fs::read_dir(&*root)).filter_map(Result::ok).any(|entry| {
        entry
            .file_name()
            .to_str()
...
fn session_store_unique_temps_survive_stale_files_and_failures() {
    for failure in ["write", "shard-rename", "prune-rename"] {
        ...
        let failed = super::columnar::append(
            &session,
            super::columnar::Stream::Messages,
            &refs,
        );
        let leaked = ok!(fs::read_dir(&store)).filter_map(Result::ok).any(|entry| {
...
fn session_store_fixed_indexes_keep_streams_independent() {
    ...
    assert_eq!(actual_messages, format!("{}\n", messages.join("\n")).as_bytes());
...
fn spawn_session_store_lock_child(
    ...
    let child = Command::new(std::env::current_exe()?)
        .arg("--exact")
        .arg("tests::session_store_lock_child")
        .arg("--nocapture")
...
fn session_store_lock_child() {
    ...
    let result = match mode.as_str() {
        "hold-exclusive" | "hold-shared" => super::columnar::with_test_store_lock(
            &session,
            mode == "hold-exclusive",
            || {
