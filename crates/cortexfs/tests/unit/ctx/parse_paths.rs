...
fn hosted_agent_guards_one_writable_system_ctx_projection() {
    ...
    if host_ready {
        ...
        assert!([["-t", "none"], ["-u", "none"], ["-T", "none"], ["-U", "none"]].iter().all(|pair| command.args.windows(2).any(|w| w == pair)));
    } else {
        assert!(command.args.iter().any(|arg| arg == "--unshare-net") && !command.args.iter().any(|arg| arg == cortexfs::support::command::PASTA));
    }
}
