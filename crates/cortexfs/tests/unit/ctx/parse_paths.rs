...
fn hosted_cli_state_target_is_exact_sibling_of_agent_home() {
    for (target, allowed) in [(HOSTED_CLI_STATE_TARGET, true), ("/home/agent/.codex", false), ("/home/cli-state/nested", false)] {
        let mount = AgentMount { source: "/tmp/state".into(), target: target.into(), mode: "rw".into() };
