...
fn schedule_result_rejects_wrong_backing_session_without_recording() {
    ...
    write_text_file(&root.join("agent/worker.d/parent"), "agent:executor session:feature\n");
    ...
    assert!(matches!(
        ...
        Err(ref error)
            if error.code == 2
                && error.message
                    == "handoff agent parent mismatch for worker: agent:executor session:feature"
    ));
    assert!(matches!(
        fs::read_to_string(child.join("status")).as_deref(),
        Ok("pending\n")
...
fn schedule_parent_binding_requires_matching_run_when_parent_run_is_known() {
    assert!(matches!(
        schedule_require_handoff_parent(
            ...
            "worker",
            "agent:executor session:default"
        ),
        Err(ref error)
            if error.code == 2
                && error.message
                    == "handoff agent parent mismatch for worker: agent:executor session:default"
    ));
    assert_eq!(
        schedule_require_handoff_parent(
            "agent:executor session:default run:r1",
