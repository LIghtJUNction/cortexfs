...
use crate::abi::authority::ToolExecutionDenial;
use crate::abi::path::is_object_name;
use crate::mount::table::{MountEntry, MountTable};
use crate::support::plain::path_metadata_no_follow;
use crate::support::toolpath::ToolPathError;

pub(super) fn tool_path_denial(error: ToolPathError) -> ToolExecutionDenial {
    match error {
...
pub(super) fn symlink_safe_metadata(path: &Path) -> std::io::Result<fs::Metadata> {
...
pub(super) fn linux_identity_can_execute(
...
pub(super) fn linux_identity_can_read(
    metadata: &fs::Metadata,
    identity: &AgentUnixIdentity,
) -> bool {
...
pub(super) fn linux_identity_can_write(
    metadata: &fs::Metadata,
    identity: &AgentUnixIdentity,
) -> bool {
...
pub(super) fn linux_identity_has_mode(
    metadata: &fs::Metadata,
    identity: &AgentUnixIdentity,
    owner_bit: u32,
    group_bit: u32,
...
pub(super) fn most_specific_mount_for_path<'a>(
...
pub(super) fn is_stable_shared_mount_for(mount: &MountEntry, shared_name: &str) -> bool {
    is_stable_shared_path_for(Path::new(mount.source()), shared_name)
        || is_stable_shared_path_for(Path::new(mount.target()), shared_name)
}

pub(super) fn is_stable_shared_path_for(path: &Path, shared_name: &str) -> bool {
...
pub(super) enum MountedSessionPath {
    Private {
        uid: u32,
        agent: String,
        session: String,
...
impl MountedSessionPath {
    ...
    pub(super) fn shared_name(&self) -> Option<&str> {
        match *self {
    ...
    pub(super) fn home_uid_allows(&self, identity: &AgentUnixIdentity) -> bool {
        match *self {
...
pub(super) fn mounted_session_path(mount: &MountEntry, path: &Path) -> Option<MountedSessionPath> {
    let stable = mounted_stable_path(mount, path)?;
    parse_mounted_session_path(&stable)
}

pub(super) fn mounted_stable_path(mount: &MountEntry, path: &Path) -> Option<PathBuf> {
...
pub(super) fn parse_mounted_session_path(path: &Path) -> Option<MountedSessionPath> {
...
pub(super) fn stable_path_parts(path: &Path) -> Option<Vec<&str>> {
