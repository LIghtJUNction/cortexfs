...
#[test]
fn channel_config_is_written_through_and_reloaded() -> fuse3::Result<()> {
    let config_dir = temp_chan_config_dir("persisted-channel");
    ...
    let config_path = config_dir.join("fengying.conf");
    let persisted = std::fs::read_to_string(&config_path).map_err(|_error| libc::EIO)?;
    assert!(persisted.contains("url=https://api.fengying.xin\n"));
    assert!(persisted.contains("keyref=env:FENGYING_API_KEY\n"));
    assert!(persisted.contains("fmt=openai.chat,openai.responses\n"));
    assert!(!persisted.contains("sk-"));

    let fs = CortexFs::new_with_chan_config_dir(config_dir.clone());
    let runtime = fs.runtime.lock().map_err(|_error| libc::EIO)?;
    let chan = runtime
        .lookup_child(chan_root, "fengying")
        .map(crate::Node::inode)
        .ok_or_else(fuse3::Errno::new_not_exist)?;
    assert_eq!(
        runtime
    ...
    assert_eq!(
        runtime
            .lookup_child(chan, "url")
            .and_then(crate::Node::content),
        Some("https://api.fengying.xin\n")
    );
    assert_eq!(
        runtime
            .lookup_child(chan, "keyref")
            .and_then(crate::Node::content),
        Some("env:FENGYING_API_KEY\n")
    );
    assert_eq!(
        runtime
            .lookup_child(chan_root, "list")
    ...
    drop(runtime);
    let _result = std::fs::remove_dir_all(config_dir);
    Ok(())
}
