...
fn responses_agent_body_declares_tsh_function_tool() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (protocol, function) in [
        ...
        assert_eq!(value.pointer(&format!("{function}/strict")), expected);
...
fn responses_runner_filters_provider_content() {
    ...
    let b = br#"{"id":"r","model":"m","content":[{"type":"thinking","thinking":"x"},{"type":"text","text":"public"}],"stop_reason":"end_turn"}"#;
