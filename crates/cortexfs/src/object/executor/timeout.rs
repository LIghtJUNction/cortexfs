...
pub(crate) fn agent_tool_timeout_seconds() -> u64 {
    agent_tool_timeout_seconds_from_env(|name| env::var(name).ok())
}

pub(crate) fn agent_model_timeout_seconds() -> u64 {
    agent_model_timeout_seconds_from_env(|name| env::var(name).ok())
}

pub(crate) fn agent_tool_timeout_seconds_from_env(get_env: impl Fn(&str) -> Option<String>) -> u64 {
...
pub(crate) fn agent_model_timeout_seconds_from_env(
