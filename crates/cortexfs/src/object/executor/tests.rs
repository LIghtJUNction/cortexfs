use super::{
    ...
    admit_provider_candidate, agent_model_timeout_seconds_from_env, agent_tool_bwrap_args,
    agent_tool_call_from_value, agent_tool_timeout_seconds_from_env, collect_child_stderr,
    curl_config_quote, execute_agent_tool_call, first_tool_call, frames_have_error,
    is_passthrough_tool, is_regular_file_no_follow, missing_model_message, model_candidates,
    nested_control_environment, nested_control_socket_is_plain, normalize_agent_model_frame,
    open_executable_no_follow, openai_api_key, openai_chat_body, openai_chat_body_with_agent_tools,
    openai_responses_body, openai_stream_event, parse_agent_context_budget,
    parse_agent_window_environment, parse_anthropic_message_content, parse_openai_chat_content,
    parse_openai_response_content, passthrough_tool_program, proc_fd_path,
    provider_config_from_dir, provider_curl_command, provider_egress_transport,
    ...
    run_core_tool_cli, run_passthrough_tool, serialized_agent_messages, spawn_child_stderr_reader,
    split_object_args, token_usage_from_value, tool_call_from_text, tool_terminal_done_line,
    tool_terminal_running_line, transport_allows_unauthenticated, trim_tool_result,
    validate_agent_tsh_args, validate_nested_control_values, visible_workspace_source,
    wait_for_curl_json_output, write_model_text_or_tool_call,
};
use cortexfs::{
...
#[cfg(unix)]
pub(crate) fn write_agent_window_fixture(root: &Path, control: &Path) -> std::io::Result<()> {
