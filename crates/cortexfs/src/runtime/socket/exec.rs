...
pub(crate) fn agent_tool_context_for_request(cwd: Option<&str>, workspace: Option<&str>) -> String {
    ...
    context.push('\n');
    match workspace.filter(|value| support::path::is_absolute_host_workspace_path(value)) {
        Some(workspace) => {
