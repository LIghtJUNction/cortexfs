...
fn parent_session_name(parent_session_dir: &Path) -> Result<String, AgentScheduleRecordError> {
    let Some(name) = parent_session_dir.file_name().and_then(|name| name.to_str()) else {
