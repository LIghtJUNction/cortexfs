fn run_umask(test: &str, child_env: &str) -> std::io::Result<std::process::Output> {
    std::process::Command::new("sh")
        .arg("-c")
        .arg(format!(
            "umask 077; exec \"$1\" tests::atomic::{test} --exact"
        ))
        .arg("sh")
...
fn atomic_replace_text_with_mode_ignores_restrictive_umask() -> std::io::Result<()> {
    ...
    let output = run_umask(
        "atomic_replace_text_with_mode_ignores_restrictive_umask",
        CHILD_ENV,
    )?;
...
#[test]
fn atomic_writers_distinguish_exact_mode_from_umask() -> std::io::Result<()> {
    ...
    if std::env::var_os(CHILD_ENV).is_some() {
        ...
        crate::support::atomic::write_file_atomic_at(&parent, "manifest.json", b"{}\n", 0o644)?;

        let path = temp.path().join("state.txt");
        assert_eq!(fs::read_to_string(&path)?, "new\n");
        assert_eq!(fs::metadata(path)?.permissions().mode() & 0o7777, 0o644);
        let manifest = fs::metadata(temp.path().join("manifest.json"))?;
        assert_eq!(manifest.permissions().mode() & 0o7777, 0o600);
        return Ok(());
    }

    let output = run_umask(
        "atomic_writers_distinguish_exact_mode_from_umask",
        CHILD_ENV,
    )?;
    assert_child_test_ran(&output);
    Ok(())
...
fn atomic_replace_text_preserving_metadata_ignores_restrictive_umask() -> std::io::Result<()> {
    ...
    let output = run_umask(
        "atomic_replace_text_preserving_metadata_ignores_restrictive_umask",
        CHILD_ENV,
    )?;
    assert_child_test_ran(&output);
    Ok(())
...
fn atomic_replace_text_preserving_metadata_refuses_readonly_target() -> std::io::Result<()> {
    ...
    if nix::unistd::Uid::effective().is_root() {
        ...
        let output = std::process::Command::new("/proc/self/exe")
            .arg("tests::atomic::atomic_replace_text_preserving_metadata_refuses_readonly_target")
            .arg("--exact")
            .env(CHILD_ENV, "1")
            .env(PATH_ENV, &path)
            .uid(UID)
...
fn atomic_replace_text_refuses_target_identity_swap_before_commit() -> std::io::Result<()> {
    ...
    let result =
        atomic_replace_text_preserving_metadata_with_hook(&path, "replacement\n", &mut hook);
    assert!(result.is_err());
...
#[test]
fn atomic_replace_text_with_mode_replaces_symlink_without_writing_target() -> std::io::Result<()> {
    let temp = tempfile::tempdir()?;
    let outside = tempfile::tempdir()?;
