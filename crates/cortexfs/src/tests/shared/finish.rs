...
fn shared_queue_finish_writes_readable_done_result_and_cleans_lease() {
    ...
    let temp_leftovers = fs::read_dir(root.join("done")).map_or(usize::MAX, |entries| {
        entries
            .filter_map(Result::ok)
            .filter(|entry| entry.file_name().to_string_lossy().contains(".result.tmp-"))
            .count()
    });
...
fn shared_queue_finish_does_not_overwrite_existing_result() {
    ...
    assert_file_text(
        &root.join("lease").join("job-1.req.json").join("worker"),
        "worker-a\n",
    );
}
use super::*;
