...
pub(crate) fn resolve_fuse_abi_path(root: &Path, abi_path: &str) -> Result<PathBuf, FuseV1Error> {
...
pub(crate) fn normalize_fuse_abi_path(abi_path: &str) -> Result<String, FuseV1Error> {
...
pub(crate) fn fuse_join_child_path(parent: &str, name: &str) -> Result<String, FuseV1Error> {
    if name.is_empty()
        || name == "."
        || name == ".."
        || name.contains('/')
...
pub(crate) fn fuse_v1_inode_for_path(abi_path: &str) -> u64 {
...
pub(crate) fn fuse_file_type(file_type: fs::FileType) -> FuseV1FileType {
    if file_type.is_dir() {
...
pub(crate) fn fuse_metadata_error(error: &std::io::Error) -> FuseV1Error {
    match error.kind() {
...
pub(crate) fn is_fuse_v1_writable_control_path(abi_path: &str) -> bool {
