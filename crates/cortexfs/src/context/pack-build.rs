...
pub(crate) fn require_pack_session_files(session_dir: &Path) -> Result<(), ContextPackBuildError> {
...
pub(crate) fn read_context_budget(path: &Path) -> Result<Option<u64>, ContextPackBuildError> {
...
pub(crate) fn append_pinned_pack_candidates(
    context: &Path,
...
pub(crate) fn append_context_file_candidate(
    context_dir: &Path,
    kind: &str,
    source: &str,
    range: Option<String>,
...
pub(crate) fn append_context_jsonl_candidate(
    context_dir: &Path,
    kind: &str,
    source: &str,
...
pub(crate) fn append_recent_messages_candidate(
    messages: &str,
    recent_message_limit: usize,
    candidates: &mut Vec<PackCandidate>,
) {
...
pub(crate) fn append_child_result_candidates(
    context: &Path,
    candidates: &mut Vec<PackCandidate>,
) -> Result<(), ContextPackBuildError> {
...
pub(crate) fn read_context_source(
    context: &Path,
    source: &str,
...
pub(crate) fn select_pack_candidates(
...
pub(crate) fn context_pack_item_json(item: &ContextPackBuiltItem) -> Value {
...
pub(crate) fn render_context_pack_markdown(
    session: &str,
    agent: Option<&str>,
    budget: Option<u64>,
    candidates: &[PackCandidate],
...
pub(crate) fn push_markdown_kv(output: &mut String, key: &str, value: &str) {
    output.push_str(key);
    output.push_str(": ");
...
#[path = "pack-build-io.rs"]
pub mod pack_build_io;
use pack_build_io::*;
