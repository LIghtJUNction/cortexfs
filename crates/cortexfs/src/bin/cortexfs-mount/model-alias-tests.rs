mod model_alias_tests {
    ...
    fn unlink_model_path_ignores_non_symlink_provider_entries() {
        ...
        assert_eq!(
            fs.unlink_model_path(INodeNo(42), OsStr::new("fixture")),
            Ok(false)
        );
    ...
    fn unlink_model_path_removes_model_alias_symlinks() {
        ...
        assert_eq!(
            fs.unlink_model_path(INodeNo(42), OsStr::new("scratch")),
            Ok(true)
        );
        assert!(!alias.exists());
    }
    ...
    fn destroy_drops_fuse_lifecycle_caches() -> Result<(), FuseV1Error> {
        ...
        assert!(
            fs.lookup_counts
                .lock()
                .is_ok_and(|counts| counts.is_empty())
        );
        assert!(
            fs.socket_overlays
                .lock()
                .is_ok_and(|sockets| sockets.is_empty())
        );
        Ok(())
    }
