fn agent_new_host_fallback(root: &Path, args: &AgentNewArgs) -> Result<ExitCode, CliError> {
    ...
    let model = args
        .models
        .first()
        .map_or_else(|| default_agent_process_model(&args.name).to_owned(), Clone::clone);
    ...
    let system = format!("You are CortexFS agent `{}`.\n", args.name);
...
fn agent_new_policy_subject(args: &AgentNewArgs) -> String {
...
fn agent_new_mount_control(uid: &str, name: &str, mounts: &[AgentMount]) -> String {
    let mut lines = vec![
...
fn agent_new_policy(subject: &str, model: &str, tools: &[String]) -> String {
...
fn write_agent_host_stub(path: &Path, name: &str) -> Result<(), CliError> {
    ...
    fs::set_permissions(path, permissions).map_err(|error| {
        CliError::unavailable(format!("cannot chmod {}: {error}", path.display()))
    })
}

fn agent_host_stub_script(name: &str) -> String {
...
fn ensure_agent_home_skeleton(root: &Path, uid: &str, name: &str) -> Result<(), CliError> {
