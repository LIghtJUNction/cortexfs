...
#[expect(
    ...
    reason = "provider OAuth dispatch reaches the socketpair client"
)]
pub(crate) fn oauth_device_login(
    provider: &str,
    profile: &str,
    ...
    let mut child = Command::new(super::runner_path()?)
        .stdin(super::socket_stdio(runner_input))
        .stdout(super::socket_stdio(runner_output))
        .stderr(Stdio::null())
        .spawn()
        .map_err(|_error| CliError::unavailable("cannot start auth runner"))?;
    let request_id = format!("auth-{}", hex_bytes(&read_system_entropy(16)?));
    let frame = cortexfs::AuthWireFrame::new(cortexfs::AuthWireRequest::Device {
...
fn read_result(stream: UnixStream, request_id: &str) -> Result<(), CliError> {
    ...
    loop {
        ...
        match frame.frame {
            cortexfs::AuthWireResponse::Progress { detail, .. } => {
                if let Some(detail) = detail {
                    print_line(&format!("open verification URL and enter code: {detail}"))?;
                }
            }
