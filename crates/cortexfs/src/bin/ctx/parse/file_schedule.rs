...
fn parse_schedule_args(args: Vec<String>) -> Result<ScheduleArgs, CliError> {
    let mut values = args.into_iter();
    let command = required_arg(&mut values, "schedule requires status, advance, claim, or result")?;
    match command.as_str() {
        "status" => {
            let path = required_arg(&mut values, "schedule status requires context/plan.json")?;
            Ok(ScheduleArgs::Status {
                path,
                done: parse_schedule_done_flags(values, "schedule status --done requires a node id")?,
            })
        }
        "advance" => {
            ...
            Ok(ScheduleArgs::Advance {
                path,
                done: parse_schedule_done_flags(values, "schedule advance --done requires a node id")?,
            })
        }
        "claim" => {
        ...
        "result" => {
            ...
            let status = required_arg(&mut values, "schedule result requires done, error, or cancelled")?;
        ...
        _ => Err(CliError::usage("schedule expects status, advance, claim, or result")),
    }
}

fn parse_schedule_done_flags(
    mut values: impl Iterator<Item = String>,
    missing_value: &str,
