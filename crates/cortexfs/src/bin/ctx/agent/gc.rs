...
pub(crate) fn agent_session_gc_delete_session_root(
    root: &Path,
    name: &str,
    session_root: &Path,
) -> PathBuf {
...
pub(crate) fn agent_session_gc_protected(
    args: &AgentSessionGcArgs,
    current: &str,
) -> HashSet<String> {
    let mut protected =
...
pub(crate) fn agent_session_gc_patterns(args: &AgentSessionGcArgs) -> Vec<String> {
...
pub(crate) fn agent_session_gc_threshold(days: u64) -> Result<SystemTime, CliError> {
    let seconds = days
...
pub(crate) fn agent_session_gc_candidates(
    session_root: &Path,
...
pub(crate) fn remove_plain_session_dir(path: &Path) -> Result<(), CliError> {
    let metadata = fs::symlink_metadata(path).map_err(|error| {
        CliError::unavailable(format!("cannot stat {}: {error}", path.display()))
    })?;
    if !metadata.is_dir() || metadata.file_type().is_symlink() {
...
pub(crate) fn session_latest_modified(path: &Path) -> Result<SystemTime, CliError> {
...
pub(crate) fn simple_glob_match(pattern: &str, value: &str) -> bool {
    if pattern == "*" {
        return true;
    }
