...
pub(crate) fn get_id_program() -> &'static str {
    ID_PROGRAM
}

pub(crate) fn id_command() -> std::process::Command {
    let mut command = std::process::Command::new(get_id_program());
    command.arg("-u").env_clear().env("PATH", "/usr/bin:/bin");
    command
}

pub(crate) fn current_uid_text() -> Result<String, String> {
...
pub(crate) fn parse_current_uid_text(output: &str) -> Result<String, &'static str> {
    let uid = output.trim();
    if uid.is_empty() {
