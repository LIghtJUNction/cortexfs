...
pub(crate) fn copy_reader_to_stdout(mut reader: impl Read) -> io::Result<()> {
...
pub(crate) fn copy_stdin_to_stream_and_shutdown(mut stream: UnixStream) -> io::Result<u64> {
...
pub(crate) fn is_terminal_disconnect(error: &io::Error) -> bool {
    matches!(
        error.kind(),
...
#[derive(Debug)]
pub(crate) struct RawTerminalMode {
    pub(crate) original: Termios,
}

impl RawTerminalMode {
    pub(crate) fn maybe_new() -> io::Result<Option<Self>> {
        let stdin = io::stdin();
...
pub(crate) fn nix_error_to_io(error: nix::errno::Errno) -> io::Error {
    io::Error::from(error)
}
