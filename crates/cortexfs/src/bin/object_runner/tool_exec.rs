...
#[derive(Debug)]
struct AgentToolSandbox {
    workspace: PathBuf,
    upper: PathBuf,
    work: PathBuf,
}

struct AgentToolBwrapArgs<'a> {
    config: &'a AgentModelRunConfig,
    tool_executable: &'a Path,
    tool_args: &'a [OsString],
    env: &'a [(String, String)],
    mount_table: &'a cortexfs::MountTable,
    cwd: &'a Path,
    sandbox: Option<&'a AgentToolSandbox>,
}

fn prepare_agent_tool_sandbox(
...
fn visible_workspace_source(workspace: PathBuf) -> PathBuf {
    if workspace.exists() {
        return workspace;
    }
...
fn agent_tool_bwrap_args(request: &AgentToolBwrapArgs<'_>) -> Vec<OsString> {
...
fn optional_dev_toolchain_bind_args() -> Vec<OsString> {
...
fn path_uses_workspace(path: &str) -> bool {
    path == "/workspace" || path.starts_with("/workspace/")
}

fn is_absolute_host_workspace_path(path: &Path) -> bool {
    path.is_absolute()
...
fn is_stable_overlay_component(value: &str) -> bool {
    !value.is_empty()
        && value.len() <= 128
        && value
            .bytes()
...
fn workspace_overlay_hash(path: &Path) -> String {
...
fn visible_mount_source(source_root: &Path, source: &str, target: &str) -> Option<String> {
...
fn bwrap_source_root_bind_args(source_root: &Path) -> Vec<OsString> {
...
fn bwrap_dir_args_for_parent(path: &str) -> Vec<OsString> {
...
fn bwrap_dir_args_for_chdir(cwd: &str) -> Vec<OsString> {
    let mut args = Vec::new();
    if !cwd.starts_with('/') {
        return args;
    }
