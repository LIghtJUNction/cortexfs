...
fn input(request_id: &str, transport: &str) -> Result<String, serde_json::Error> {
    serde_json::to_string(&InteractionFrame::request(InteractionRequest::input(
        request_id,
        "default",
        "hello",
        InteractionOrigin {
...
fn post(socket: PathBuf, request_id: &str) -> Result<HttpResponse, Box<dyn std::error::Error>> {
    let request = HttpRequest {
        method: "POST".into(),
        path: "/v1/interaction".into(),
        headers: BTreeMap::new(),
        body: input(request_id, "web")?,
    };
    Ok(handle(
        &WebConfig {
            socket,
...
) -> std::io::Result<(SocketAddr, thread::JoinHandle<std::io::Result<()>>)> {
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let response = post(socket, "web-1")?;
    worker
        .join()
...
fn web_host_rejects_interactive_commands_without_hanging() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let response = post(socket, "web-command-1")?;
    worker
        .join()
        .map_err(|_error| std::io::Error::other("web worker panicked"))??;
    assert_eq!(response.status, 200);
...
fn websocket_host_returns_command_and_accepts_reply() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (address, server) = server(agent_socket)?;
    let (mut client, _) = tungstenite::connect(format!("ws://{address}/v1/interaction"))?;
    client.send(tungstenite::Message::text(input("ws-1", "websocket")?))?;
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let (address, server) = server(agent_socket)?;
    let (mut client, _) = tungstenite::connect(format!("ws://{address}/v1/interaction"))?;
    client.send(tungstenite::Message::text(input(
        "ws-control-1",
        "websocket",
    )?))?;
