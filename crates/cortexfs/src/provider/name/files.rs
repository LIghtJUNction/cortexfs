use super::names::MAX_PROVIDER_SYSTEM_SECRET_BYTES;
use crate::*;
...
pub(crate) fn open_provider_secret_file(path: &Path) -> std::io::Result<File> {
...
pub(crate) fn create_private_provider_secret_dir(
    path: &Path,
...
pub(crate) fn clear_fd_cloexec(file: &File) -> Result<(), ProviderSystemSecretError> {
    let flags =
...
pub(crate) fn provider_env_label(provider: &str) -> String {
    provider
        .bytes()
        .map(|byte| match byte {
