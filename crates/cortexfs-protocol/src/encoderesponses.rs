...
pub(super) fn request(request: &ModelRequest) -> Result<Vec<u8>, ConversionError> {
    ...
    if !request.tools.is_empty() {
        root.insert("tools".to_owned(), Value::Array(request.tools.iter().map(|tool| json!({"type": "function", "name": tool.name, "description": tool.description, "parameters": tool.parameters, "strict": null})).collect()));
    }
