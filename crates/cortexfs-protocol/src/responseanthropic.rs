...
fn block_events(events: &mut Vec<ModelEvent>, run: &str, value: &Value) -> Result<(), ConversionError> {
    ...
    match crate::responseutil::text(map.get("type")).as_deref() {
        ...
        Some("tool_use") => events.push(ModelEvent::ToolCall {
            run: run.to_owned(),
            call: crate::ToolCall {
                id: crate::responseutil::text(map.get("id"))
                    .ok_or_else(|| invalid("content[].id"))?,
                name: crate::responseutil::text(map.get("name"))
                    .ok_or_else(|| invalid("content[].name"))?,
                arguments: map
                    .get("input")
                    .filter(|value| value.is_object())
                    .cloned()
                    .ok_or_else(|| invalid("content[].input"))?,
            },
        }),
        _ => {}
    }
...
pub(super) fn encode(events: &[ModelEvent]) -> Result<Vec<u8>, ConversionError> {
    ...
    let mut root = json!({
        "id": summary.run, "model": summary.model, "role": "assistant", "content": content,
        "stop_reason": crate::responseutil::finish(summary.status),
    });
