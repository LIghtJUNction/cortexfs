...
当前 ABI 内未启用编程式工具调用。CortexFS 不应宣告 OpenAI `programmatic_tool_calling` 工具、`allowed_callers`，也不应把 `schema` 当作 `output_schema`。当前仍保持 single-call host loop 作为唯一 executable-agent 工具协议。
