...
| Source installer still mentions `agent/coder.d/model` | Optional provider and IM onboarding in `scripts/install-linux.sh` still starts leftover `coder` and binds its model control. Bind `agent/executor.d/model` and point channels at `/ctx/agent/main.sock` or `/ctx/agent/executor.sock`. |
